...
module.exports = function (app,User,mongoose,session) {
    ...
    app.post('/search', (req, res) => {
        ...
        var query = {
            email: {$ne: req.session.user[0].email},
            status: {$ne: "NotAvailable"},
            min_budget: {$lte: minAmt},
            max_budget: {$gte: maxAmt}
        };
    ...
    app.get('/search', (req, res) => {
        User.find({email: emailsess}, (err, docs) => {
            ...
            var query = {
                email: {$ne: currUser.email},
                status: {$ne: "NotAvailable"},
                room_sharing: currUser.room_sharing,
                min_budget: {$lte: currUser.max_budget},
                max_budget: {$gte: currUser.min_budget},
                location: {$in: currUser.location}
            };
            ...
            for (var i = 0, len = arr.length; i < len; i++) {
                ...
                results_arr.push(User.find(querymod, function (errors, docs) {
        ...
        User.find({email: req.session.user[0].email}, (err, docs) => {
            if (err) {
                ...
                User.find({email: emailsess}, (errs, resp) => {
                    ...
                    updatedchar = true;
                    res.render('userProfile/index', {
                        flag: hasupdatedCharacteristics,
                        flagger: updatedchar,
                        hasloggedin: hasloggedin,
                        usersession: resp[0]
                    });
